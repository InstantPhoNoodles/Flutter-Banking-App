...
class PayCreditBox extends StatefulWidget {
  final Account creditAccount; // the card being paid off

  const PayCreditBox({super.key, required this.creditAccount});

  @override
  State<PayCreditBox> createState() => _PayCreditBoxState();
}

class _PayCreditBoxState extends State<PayCreditBox> {
  ...
  void _submit() {
    ...
    debugPrint('Raw input: "${_amountController.text}" | Parsed: $amount');
     ...
     if (_fromAccount == null) {
      _showSnackbar('Please select an account.', success: false);
      return;
    }
    if (amount <= 0) {
    ...
    if (amount > widget.creditAccount.balance) {
      _showSnackbar('Amount exceeds current balance on this card.', success: false);
      return;
    }

    final success = context.read<AccountsProvider>().payCredit(
      fromAcnt: _fromAccount!.accountNumber,
      toAcnt: widget.creditAccount.accountNumber,
      amount: amount,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text('Pay Credit Card', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
        SizedBox(height: 5),
